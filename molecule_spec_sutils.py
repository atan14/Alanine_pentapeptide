...
class Sutils(object):
    ...
    def prepare_training_data_using_Cartesian_coordinates_with_data_augmentation(
        ...
        print (data_set.shape[0], output_data_set.shape[0])
    ...
    def create_subclass_instance_using_name(name):
        ...
        if name == 'Alanine_dipeptide':
            ...
            return Trp_cage()
        elif name == 'Alanine_pentapeptide':
            return Alanine_pentapeptide()
        else:
...
class Alanine_pentapeptide(Sutils):
    """docstring for Alanine_pentapeptide"""

    def __init__(self):
        super(Alanine_pentapeptide, self).__init__()
        return
    ...
    def get_cossin_of_a_dihedral_from_four_atoms(coord_1, coord_2, coord_3, coord_4):
        ...
        coords_of_four = np.array([coord_1, coord_2, coord_3, coord_4])
        num_of_coordinates = 4
        diff_coordinates = coords_of_four[1:num_of_coordinates, :] - coords_of_four[0:num_of_coordinates - 1,
                                                                     :]  # bond vectors
        diff_coordinates_1 = diff_coordinates[0:num_of_coordinates - 2, :];
        diff_coordinates_2 = diff_coordinates[1:num_of_coordinates - 1, :]
        normal_vectors = np.cross(diff_coordinates_1, diff_coordinates_2)
        normal_vectors_normalized = np.array(map(lambda x: x / sqrt(np.dot(x, x)), normal_vectors))
        normal_vectors_normalized_1 = normal_vectors_normalized[0:num_of_coordinates - 3, :];
        normal_vectors_normalized_2 = normal_vectors_normalized[1:num_of_coordinates - 2, :]
        diff_coordinates_mid = diff_coordinates[
                               1:num_of_coordinates - 2]  # these are bond vectors in the middle (remove the first and last one), they should be perpendicular to adjacent normal vectors
        ...
        sin_of_angle = sqrt(np.dot(sin_of_angle_vec, sin_of_angle_vec)) * np.sign(
            sin_of_angle_vec[component_index] * diff_coordinates_mid[index][component_index])
        try:
            assert (cos_of_angle ** 2 + sin_of_angle ** 2 - 1 < 0.0001)
        except:
            print ("error: cos^2 x+ sin^2 x != 1, it is %f" % (cos_of_angle ** 2 + sin_of_angle ** 2))
    ...
    def get_cossin_from_a_coordinate(a_coordinate):
        # FIXME: how to write unit test for this function?
        # TODO: to be tested
        total_num_of_residues = 5
        list_of_idx_four_atoms = map(lambda x: [[3 * x - 1, 3 * x, 3 * x + 1, 3 * x + 2],
                                                [3 * x, 3 * x + 1, 3 * x + 2, 3 * x + 3]],
                                     list(range(total_num_of_residues)))
        list_of_idx_four_atoms = reduce(lambda x, y: x + y, list_of_idx_four_atoms)
        list_of_idx_four_atoms = filter(lambda x: x[0] >= 0 and x[3] < 3 * total_num_of_residues,
                                        list_of_idx_four_atoms)

        assert (len(list_of_idx_four_atoms) == 8)

        result = []

        for item in list_of_idx_four_atoms:
            parameter_list = map(
                lambda x: Alanine_pentapeptide.get_coordinates_of_atom_with_index(a_coordinate, x),
                item
            )
            [cos_value, sin_value] = Alanine_pentapeptide.get_cossin_of_a_dihedral_from_four_atoms(*parameter_list)
            # print(item)
            # print(cos_value, sin_value)
            result += [cos_value, sin_value]
    ...
    @staticmethod
    def get_many_cossin_from_coordinates(coordinates):
        return map(Alanine_pentapeptide.get_cossin_from_a_coordinate, coordinates)
    ...
    def get_many_cossin_from_coordinates_in_list_of_files(list_of_files, step_interval=1):
        ...
        coordinates = coordinates[::step_interval]
        result = Alanine_pentapeptide.get_many_cossin_from_coordinates(coordinates)
    ...
    def get_many_dihedrals_from_coordinates_in_file(list_of_files, step_interval=1):
        # why we need to get dihedrals from a list of coordinate files?
        # because we will probably need to plot other files outside self._list_of_coor_data_files
        temp = Alanine_pentapeptide.get_many_cossin_from_coordinates_in_list_of_files(list_of_files, step_interval)
        return Alanine_pentapeptide.get_many_dihedrals_from_cossin(temp)
    ...
    def get_pairwise_distance_matrices_of_alpha_carbon_bak(list_of_files,
                                                           step_interval=1
                                                           # get_matrices every "step_interval" snapshots
                                                           ):
        distances_list = []
        index = 0
        num_of_residues = 5
    ...
    def get_non_repeated_pairwise_distance_as_list_of_alpha_carbon(list_of_files, step_interval=1):
        """each element in this result is a list, not a matrix"""
        dis_matrix_list = Alanine_pentapeptide.get_pairwise_distance_matrices_of_alpha_carbon(list_of_files, step_interval)
        num_of_residues = 5
        result = []
        for mat in dis_matrix_list:
            p_distances = []
    ...
    @staticmethod
    def metric_get_diff_pairwise_distance_matrices_of_alpha_carbon(list_of_files, ref_file='../resources/Alanine_pentapeptide.pdb',
                                                                   step_interval=1):
        ref = Alanine_pentapeptide.get_pairwise_distance_matrices_of_alpha_carbon([ref_file])
        sample = Alanine_pentapeptide.get_pairwise_distance_matrices_of_alpha_carbon(list_of_files, step_interval)
        diff = map(lambda x: np.linalg.norm(ref[0] - x), sample)
        return diff
        ...
        for temp_file in list_of_files:
            ...
            for _ in temp_universe.trajectory:
                if index % step_interval == 0:
                    atom_list = [temp_universe.select_atoms('name CA and resid %d' % item).positions[0]
                                 for item in [1, 9, 14, 20]]
                    result.append(Alanine_pentapeptide.get_cossin_of_a_dihedral_from_four_atoms(
                        atom_list[0], atom_list[1], atom_list[2], atom_list[3])[1])
                index += 1
        return np.array(result)

    @staticmethod
    def metric_get_number_of_native_contacts(list_of_files, ref_file='../resources/Alanine_pentapeptide.pdb', threshold=8,
                                             step_interval=1):
        ref = Alanine_pentapeptide.get_pairwise_distance_matrices_of_alpha_carbon([ref_file])
        sample = Alanine_pentapeptide.get_pairwise_distance_matrices_of_alpha_carbon(list_of_files, step_interval)
    ...
    @staticmethod
    def metric_RMSD_of_atoms(list_of_files, ref_file='../resources/Alanine_pentapeptide.pdb', atom_selection_statement="name CA",
                             step_interval=1):
        """
        :param atom_selection_statement:  could be either
         - "name CA" for alpha-carbon atoms only
        ...
        for sample_file in list_of_files:
            ...
            for _ in sample.trajectory:
                if index % step_interval == 0:
                    result_rmsd_of_atoms.append(Alanine_pentapeptide.get_RMSD_after_alignment(ref_atom_selection.positions,
                                                                                  sample_atom_selection.positions))
    ...
    @staticmethod
    def get_pairwise_RMSD_after_alignment_for_a_file(sample_file, atom_selection_statement='name CA'):
        sample_1 = Universe(sample_file);
        sample_2 = Universe(
            sample_file)  # should use two variables here, otherwise it will be 0, might be related to iterator issue?
        sel_1 = sample_1.select_atoms(atom_selection_statement);
        sel_2 = sample_2.select_atoms(atom_selection_statement)

        return [[rmsd(sel_1.positions, sel_2.positions, center=True, superposition=True) for _2 in sample_2.trajectory]
                for _1 in sample_1.trajectory]
    ...
    def structure_clustering_in_a_file(sample_file, atom_selection_statement='name CA',
        ...
        pairwise_RMSD = Alanine_pentapeptide.get_pairwise_RMSD_after_alignment_for_a_file(sample_file,
                                                                              atom_selection_statement=atom_selection_statement)
        from sklearn.cluster import DBSCAN

        dbscan_obj = DBSCAN(metric='precomputed', eps=eps, min_samples=min_num_of_neighboring_samples).fit(
            pairwise_RMSD)
        class_labels = dbscan_obj.labels_
        max_class_label = max(class_labels)
        num_in_each_class = {label: np.where(class_labels == label)[0].shape[0] for label in
                             range(-1, max_class_label + 1)}
        most_common_class_labels = sorted(num_in_each_class.keys(), key=lambda x: num_in_each_class[x], reverse=True)
        with open(sample_file, 'r') as in_file:
            content = [item for item in in_file.readlines() if not 'REMARK' in item]
            content = ''.join(content)
    ...
    def rotating_dihedral_angles_and_save_to_pdb(input_pdb, target_dihedrals, output_pdb):
        pdb_parser = PDB.PDBParser(QUIET=True)
        temp_structure = pdb_parser.get_structure('temp', input_pdb)
        coor_file = Alanine_pentapeptide.generate_coordinates_from_pdb_files(input_pdb)[0]
        current_dihedrals = Alanine_pentapeptide.get_many_dihedrals_from_coordinates_in_file([coor_file])
        rotation_angles = np.array(target_dihedrals) - np.array(current_dihedrals)

        atom_indices_in_each_residue = [[]] * 5
        temp_model = list(temp_structure.get_models())[0]
        for _1, item in list(enumerate(temp_model.get_residues())):
            atom_indices_in_each_residue[_1] = [int(_2.get_serial_number()) - 1 for _2 in item.get_atom()]
        ...
        for temp_model in temp_structure.get_models():
            ...
            for item in range(12):  # 12 * 2 = 24 dihedrals in total
                C_atom_in_this_residue = filter(lambda x: x.get_name() == "C", atoms_in_this_frame)[item]
                CA_atom_in_this_residue = filter(lambda x: x.get_name() == "CA", atoms_in_this_frame)[item]
                CA_atom_in_next_residue = filter(lambda x: x.get_name() == "CA", atoms_in_this_frame)[item + 1]
                N_atom_in_next_residue = filter(lambda x: x.get_name() == "N", atoms_in_this_frame)[item + 1]
                ...
                temp_coords = Sutils.rotating_group_of_atoms(temp_coords, indices_atom_to_rotate, fixed_coord_0,
                                                             axis_vector_0,
                                                             rotation_angles[temp_model.get_id()][2 * item])
                temp_coords = Sutils.rotating_group_of_atoms(temp_coords, indices_atom_to_rotate, fixed_coord_1,
                                                             axis_vector_1,
                                                             rotation_angles[temp_model.get_id()][2 * item + 1])
    ...
    def get_expression_for_input_of_this_molecule():
        index_of_backbone_atoms = [str(item) for item in CONFIG_57[1]]
        total_num_of_residues = 5
        list_of_idx_four_atoms = map(lambda x: [3 * x, 3 * x + 1, 3 * x + 2, 3 * x + 3],
                                     list(range(total_num_of_residues))) \
                                 + map(lambda x: [3 * x - 1, 3 * x, 3 * x + 1, 3 * x + 2],
                                       list(range(total_num_of_residues)))
        list_of_idx_four_atoms = filter(lambda x: x[0] >= 0 and x[3] < 3 * total_num_of_residues,
                                        list_of_idx_four_atoms)
        assert (len(list_of_idx_four_atoms) == 8)
