...
CONFIG_30 = "Alanine_pentapeptide"     # the type of molecule we are studying, Alanine_dipeptide, or Trp_cage
...
def get_mol_param(parameter_list, molecule_name=CONFIG_30):   # get molecule specific parameter using a parameter list
    ...
    elif molecule_name == "Alanine_pentapeptide": return parameter_list[2]
...
if CONFIG_48 == 'Cartesian':
    CONFIG_49 = get_mol_param([5.0, 20.0, 10.0]) # scaling factor for output for Cartesian coordinates
...
CONFIG_57 = [
    ...
    [1, 5, 11, 13, 15, 21, 23, 25, 31, 33, 35, 41, 43, 46, 48]
...
if CONFIG_45 == 'pybrain':
    ...
    CONFIG_4 = get_mol_param([
        ...
        [0.5, 0.5, 0, True, [0.00, 0.0000, 0.00, 0.00]]
...
if CONFIG_17[2] == CircularLayer:
    CONFIG_37 = 2 * CONFIG_36              # number of nodes in bottleneck layer
elif CONFIG_17[2] == TanhLayer or CONFIG_17[2] == ReluLayer:
    CONFIG_37 = CONFIG_36
else:
...
CONFIG_62 = get_mol_param([
    ...
    ['../resources/Alanine_pentapeptide.pdb']
...
if CONFIG_48 == 'cossin':
    ...
    CONFIG_3 = get_mol_param([
         ...
         [3 * len(CONFIG_57[2]), 50, CONFIG_37, 50, 3 * len(CONFIG_57[2]) * CONFIG_55]
...
CONFIG_13 = get_mol_param([3,3,4])  # num of network trainings we are going to run, and pick the one with least FVE from them
...
CONFIG_56 = get_mol_param([20, 8, 10])    # number of biased simulations running in parallel
...
CONFIG_10 = get_mol_param([10,10,4])   # num of bins for get_boundary_points()
CONFIG_11 = get_mol_param([15,20,20])  # num of boundary points
...
elif CONFIG_17[2] == TanhLayer:
    CONFIG_18 = False
    CONFIG_26 = [[-1, 1] for item in range(CONFIG_36)]
elif CONFIG_17[2] == ReluLayer:
    CONFIG_18 = False
...
CONFIG_33 = CONFIG_3[2]   # length of list of cos/sin values, equal to the number of nodes in input layer
CONFIG_12 = '../target/' + CONFIG_30  # folder that contains all pdb files

CONFIG_65 = "US"          # default biasing method
CONFIG_16 = get_mol_param([500, 2000, 1000])                     # record interval (the frequency of writing system state into the file)
CONFIG_8 = get_mol_param([50000, 200000, 100000])                  # num of simulation steps
...
CONFIG_54 = 2.47 * get_mol_param([30.0, 15.0, 25.0])             # max external potential energy allowed (in k_BT)
...
if temp_home_directory == "/home/fisiksnju":
    ...
    CONFIG_25 = '/home/atan14/anaconda2/lib/plugins'  # this is the directory where the plugin is installed
elif temp_home_directory == "/home/atan14":
...
CONFIG_27 =  map(lambda x: layer_type_to_name_mapping[x], CONFIG_17[:3]) # layer_types for ANN_Force, it should be consistent with autoencoder
...
CONFIG_32 = 8000           # maximum force constant allowed (for force constant adjustable mode)
